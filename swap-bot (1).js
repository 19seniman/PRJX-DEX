 ...
 *    AMOUNT_USDT0=0.01          # jumlah USDT0 yang ingin di-swap ke USDH (HyperEVM)
 *    AMOUNT_USDT_BASE=0.011698  # jumlah USDT yang ingin di-swap ke USDC (Base)
 *    # Min output sudah hardcoded: USDH min=0.01, USDC min=0.011698
 *
 *  PERINGATAN:
...
// ─────────────────────────────────────────────
async function swapUSDT0toUSDH(amountIn) {
  console.log("\n" + "═".repeat(60));
  ...
  // ── Batas minimum output USDH = tepat 0.01 (hardcoded) ──
  // Swap akan revert/gagal jika output USDH yang diterima kurang dari nilai ini
  const USDH_MIN_OUT = "0.01";
  const amountOutMinimum = ethers.parseUnits(USDH_MIN_OUT, usdhDecimals);
  ...
  console.log(`     Output min : ${formatAmount(amountOutMinimum, usdhDecimals, usdhSymbol)} (hardcoded)`);
...
// ─────────────────────────────────────────────
async function swapUSDTtoUSDC_Base(amountIn) {
  console.log("\n" + "═".repeat(60));
  ...
  // ── Batas minimum output USDC = tepat 0.011698 (hardcoded) ──
  // Swap akan revert/gagal jika output USDC yang diterima kurang dari nilai ini
  const USDC_MIN_OUT = "0.011698";
  const amountOutMinimum = ethers.parseUnits(USDC_MIN_OUT, usdcDecimals);

  console.log(`\n  🔄 Detail Swap:`);
  console.log(`     Input      : ${formatAmount(amountInWei, usdtDecimals, usdtSymbol)}`);
  console.log(`     Output min : ${formatAmount(amountOutMinimum, usdcDecimals, usdcSymbol)} (hardcoded)`);
...
async function main() {
  ...
  if (!process.env.PRIVATE_KEY) {
    ...
    console.log("    AMOUNT_USDT0=0.01");
    console.log("    AMOUNT_USDT_BASE=0.011698");
    console.log("    # Min output hardcoded: USDH=0.01, USDC=0.011698");
    process.exit(1);
  }

  const amountUsdt0 = parseFloat(process.env.AMOUNT_USDT0 || "0.01");
  const amountUsdtBase = parseFloat(process.env.AMOUNT_USDT_BASE || "0.011698");
  ...
  switch (mode) {
    ...
    case "swap1":
      await swapUSDT0toUSDH(amountUsdt0);
      break;

    case "swap2":
      await swapUSDTtoUSDC_Base(amountUsdtBase);
      break;
